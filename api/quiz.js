...
const ensureTables = async () => {
    ...
    // Handle Scores columns
    for (const columnSQL of scoreColumns) {
        try {
            await sql`${sql.unsafe(columnSQL)}`;
        } catch (colError) {
            console.log('Scores column migration skipped (may exist):', colError.message);
        }
    }
    ...
    for (const columnSQL of statusColumns) {
        try {
            ...
            console.log('QuizStatus column migration skipped (may exist):', colError.message);
        }
    }
    ...
    try {
        ...
        console.log('QuizStatus insert/update error (may be fine):', error.message);
    }
};
...
export default async function handler(req, res) {
    ...
    try {
        await ensureTables(); // Always run migration first
